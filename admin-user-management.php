        ...
        <main class="main-content">
            <div class="page-container mgmt-page">
                ...
                <?php if (isset($_SESSION['success'])): ?>
                    <div class="mgmt-alert mgmt-alert-success"><i
                            class="fas fa-check-circle"></i><?php echo htmlspecialchars($_SESSION['success']);
                            unset($_SESSION['success']); ?>
                    </div>
                <?php endif; ?>
                <?php if (!empty($error)): ?>
                    <div class="mgmt-alert mgmt-alert-error"><i
                            class="fas fa-exclamation-triangle"></i><?php echo htmlspecialchars($error); ?></div>
                <?php endif; ?>
                ...
                <div class="mgmt-hero">
                    ...
                    <div class="mgmt-hero-right">
                        <button class="btn-premium-add" id="addUserBtn"><i class="fas fa-user-plus"></i> Add New
                            User</button>
                    </div>
                </div>
                ...
                <div class="mgmt-stats">
                    <div class="mgmt-stat stat-total">
                    ...
                    <div class="mgmt-stat stat-staff">
                        <div class="mgmt-stat-icon"><i class="fas fa-user-shield"></i></div>
                        <div class="mgmt-stat-info">
                            <h4><?php echo $stats['staff_count'] ?? 0; ?></h4>
                            <p>Dieticians</p>
                        </div>
                    </div>
                    <div class="mgmt-stat stat-admin">
                        <div class="mgmt-stat-icon"><i class="fas fa-user-cog"></i></div>
                        <div class="mgmt-stat-info">
                            <h4><?php echo $stats['admin_count'] ?? 0; ?></h4>
                            <p>Administrators</p>
                        </div>
                    </div>
                    <div class="mgmt-stat stat-active">
                        <div class="mgmt-stat-icon"><i class="fas fa-user-check"></i></div>
                        <div class="mgmt-stat-info">
                            <h4><?php echo $stats['regular_count'] ?? 0; ?></h4>
                            <p>Regular Users</p>
                        </div>
                    </div>
                    <div class="mgmt-stat stat-deleted">
                        <div class="mgmt-stat-icon"><i class="fas fa-user-times"></i></div>
                        <div class="mgmt-stat-info">
                            <h4><?php echo $stats['deleted_users'] ?? 0; ?></h4>
                            <p>Deleted</p>
                        </div>
                    </div>
                </div>
                ...
                <!-- Tabs -->
                <div class="mgmt-tabs">
                    <button class="mgmt-tab-btn <?php echo $current_tab === 'active_users' ? 'active' : ''; ?>"
                        onclick="window.location='?tab=active_users'"><i class="fas fa-users"></i> Active Users</button>
                    <button class="mgmt-tab-btn <?php echo $current_tab === 'delete_history' ? 'active' : ''; ?>"
                        onclick="window.location='?tab=delete_history'"><i class="fas fa-history"></i> Delete History
                        <?php if ($stats['deleted_users'] > 0)
                            echo '<span style="background:#f43f5e;color:white;border-radius:50px;padding:1px 8px;font-size:0.72rem;">' . $stats['deleted_users'] . '</span>'; ?></button>
                </div>
                ...
                <div class="mgmt-tab-panel <?php echo $current_tab === 'active_users' ? 'active' : ''; ?>">
                    <div class="mgmt-toolbar">
                        ...
                        <div class="mgmt-search"><i class="fas fa-search"></i><input type="text"
                                placeholder="Search users..." oninput="filterCards(this.value,'profileGrid')"></div>
                    </div>
                    <?php if (empty($users)): ?>
                        <div class="mgmt-empty">
                            <div class="mgmt-empty-icon"><i class="fas fa-users"></i></div>
                    ...
                    <?php else: ?>
                        <div class="profile-grid" id="profileGrid">
                            <?php foreach ($users as $user):
                                $roleIcon = $user['role'] === 'admin' ? 'crown' : ($user['role'] === 'staff' ? 'user-shield' : 'user'); ?>
                                <div class="profile-card role-<?php echo $user['role']; ?> status-<?php echo $user['status']; ?>"
                                    data-search="<?php echo strtolower($user['name'] . ' ' . $user['email'] . ' ' . $user['role']); ?>">
                                    <div class="profile-card-accent"></div>
                                    <div class="profile-card-body">
                                        <div class="profile-avatar role-<?php echo $user['role']; ?>">
                                            <?php echo getInitials($user['name']); ?><span
                                                class="status-dot <?php echo $user['status'] === 'active' ? 'active-dot' : ''; ?>"></span>
                                        </div>
                                        <div class="profile-name"><?php echo htmlspecialchars($user['name']); ?></div>
                                        <div class="profile-id">#USR-<?php echo str_pad($user['id'], 3, '0', STR_PAD_LEFT); ?>
                                        </div>
                                        <div class="profile-email"><i
                                                class="fas fa-envelope"></i><?php echo htmlspecialchars($user['email']); ?>
                                        </div>
                                        <div class="profile-badges">
                                            <span class="profile-badge badge-role-<?php echo $user['role']; ?>"><i
                                                    class="fas fa-<?php echo $roleIcon; ?>"></i>
                                                <?php echo getUserRoleText($user['role']); ?></span>
                                            <span class="profile-badge badge-<?php echo $user['status']; ?>"><i
                                                    class="fas fa-circle"></i> <?php echo ucfirst($user['status']); ?></span>
                                        </div>
                                        <div class="profile-meta"><i class="fas fa-calendar-alt"></i> Joined
                                            <?php echo date('M j, Y', strtotime($user['created_at'])); ?></div>
                                    </div>
                                    <div class="profile-card-footer">
                                        <form method="POST" class="inline-form" style="flex:1;"><input type="hidden"
                                                name="action" value="update_role"><input type="hidden" name="user_id"
                                                value="<?php echo $user['id']; ?>"><select name="role"
                                                onchange="this.form.submit()" class="card-role-select">
                                                <option value="regular" <?php echo $user['role'] === 'regular' ? 'selected' : ''; ?>>
                                                    Regular</option>
                                                <option value="staff" <?php echo $user['role'] === 'staff' ? 'selected' : ''; ?>>Dietician
                                                </option>
                                        ...
                                        <form method="POST" class="inline-form"><input type="hidden" name="action"
                                                value="toggle_status"><input type="hidden" name="user_id"
                                                value="<?php echo $user['id']; ?>"><button type="submit"
                                                class="card-action toggle" title="Toggle Status"><i
                                                    class="fas fa-power-off"></i></button></form>
                                        <button type="button" class="card-action edit edit-btn"
                                            data-user-id="<?php echo $user['id']; ?>"
                                            data-name="<?php echo htmlspecialchars($user['name']); ?>"
                                            data-email="<?php echo htmlspecialchars($user['email']); ?>"
                                            data-role="<?php echo $user['role']; ?>"
                                            data-status="<?php echo $user['status']; ?>" title="Edit"><i
                                                class="fas fa-pen"></i></button>
                                        <form method="POST" class="inline-form" onsubmit="return confirm('Move to trash?')">
                                            <input type="hidden" name="action" value="delete_user"><input type="hidden"
                                                name="user_id" value="<?php echo $user['id']; ?>"><button type="submit"
                                                class="card-action delete" title="Delete"><i class="fas fa-trash"></i></button>
                                        </form>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                ...
                <div class="mgmt-tab-panel <?php echo $current_tab === 'delete_history' ? 'active' : ''; ?>">
                    <div class="mgmt-section-label" style="margin-bottom:20px;"><i class="fas fa-history"></i> Deleted
                        User Archive</div>
                    <?php if (empty($deleted_users)): ?>
                        <div class="mgmt-empty">
                            <div class="mgmt-empty-icon"><i class="fas fa-history"></i></div>
                            <h3>No deleted users</h3>
                            <p>Deleted users are kept for 30 days before being purged.</p>
                        </div>
                    <?php else: ?>
                        <div class="deleted-grid">
                            <?php foreach ($deleted_users as $du):
                                $age = floor((time() - strtotime($du['deleted_at'])) / 86400);
                                $canPerm = $age >= 10; ?>
                                <div class="deleted-card">
                                    <div class="deleted-avatar"><?php echo getInitials($du['name']); ?></div>
                                    <div class="deleted-info">
                                        <div class="deleted-name"><?php echo htmlspecialchars($du['name']); ?> <span
                                                style="font-size:0.75rem;color:#9ca3af;">#USR-<?php echo str_pad($du['original_id'], 3, '0', STR_PAD_LEFT); ?></span>
                                        </div>
                                        <div class="deleted-meta">
                                            <span><?php echo htmlspecialchars($du['email']); ?></span>
                                            <span
                                                class="profile-badge badge-role-<?php echo $du['role']; ?>"><?php echo getUserRoleText($du['role']); ?></span>
                                            <div class="deleted-by-chip"><i
                                                    class="fas fa-user-times"></i><?php echo htmlspecialchars($du['deleted_by_name']); ?>
                                            </div>
                                            <span><?php echo date('M j, Y', strtotime($du['deleted_at'])); ?></span>
                                        </div>
                                    </div>
                                    <div class="deleted-actions">
                                        <form method="POST" class="inline-form" onsubmit="return confirm('Restore this user?')">
                                            <input type="hidden" name="action" value="restore_user"><input type="hidden"
                                                name="deleted_user_id" value="<?php echo $du['id']; ?>"><button type="submit"
                                                class="card-action restore" title="Restore"><i class="fas fa-undo"></i></button>
                                        </form>
                                        <form method="POST" class="inline-form"
                                            onsubmit="return confirm('Permanently delete? Cannot be undone.')"><input
                                                type="hidden" name="action" value="permanent_delete"><input type="hidden"
                                                name="deleted_user_id" value="<?php echo $du['id']; ?>"><button type="submit"
                                                class="card-action delete perm-delete-btn"
                                                data-deleted-at="<?php echo $du['deleted_at']; ?>" data-min-days="10" <?php echo $canPerm ? '' : 'disabled'; ?> title="Permanent Delete"><i
                                                    class="fas fa-skull"></i></button></form>
                                        <span class="countdown-chip perm-countdown"
                                            data-deleted-at="<?php echo $du['deleted_at']; ?>" data-min-days="10"></span>
                                    </div>
                                </div>
                            <?php endforeach; ?>
                        </div>
    ...
    <div class="premium-modal-overlay" id="addUserModal">
        <div class="premium-modal">
            ...
            <form method="POST" action="process_admin_add_user.php">
                ...
                <div class="premium-form-group"><label>Full Name</label><input type="text" name="name" required
                        placeholder="Enter full name"></div>
                <div class="premium-form-group"><label>Email Address</label><input type="email" name="email" required
                        placeholder="Enter email"></div>
                <div class="premium-form-group"><label>Password</label><input type="password" name="password" required
                        placeholder="Enter password"></div>
                <div class="premium-form-group"><label>Confirm Password</label><input type="password"
                        name="confirm_password" required placeholder="Confirm password"></div>
                <div class="premium-form-group"><label>Role</label><select name="role" required>
                        <option value="regular">Regular User</option>
                        <option value="staff">Dietician</option>
                        <option value="admin">Administrator</option>
                    </select></div>
                <div class="premium-modal-actions">
                    <button type="button" class="btn-modal-cancel"
                        onclick="document.getElementById('addUserModal').classList.remove('active');document.body.style.overflow='auto'">Cancel</button>
    ...
    <div class="premium-modal-overlay" id="editUserModal">
        <div class="premium-modal">
            ...
            <form method="POST" id="editUserForm">
                ...
                <div class="premium-form-group"><label>Full Name</label><input type="text" id="edit_name" name="name"
                        required></div>
                <div class="premium-form-group"><label>Email</label><input type="email" id="edit_email" name="email"
                        required></div>
                <div class="premium-form-group"><label>Role</label><select id="edit_role" name="role">
                        <option value="regular">Regular User</option>
                        <option value="staff">Dietician</option>
                        <option value="admin">Administrator</option>
                    </select></div>
                <div class="premium-form-group"><label>Status</label><select id="edit_status" name="status">
                        <option value="active">Active</option>
                        <option value="inactive">Inactive</option>
                    </select></div>
                <div class="premium-form-group"><label>New Password <span
                            style="color:#9ca3af;font-weight:400;text-transform:none;">(optional)</span></label><input
                        type="password" id="edit_password" name="password" placeholder="Leave blank to keep current">
                </div>
                <div class="premium-form-group"><label>Confirm New Password</label><input type="password"
                        id="edit_confirm_password" name="confirm_password" placeholder="Re-enter new password"></div>
                <div class="premium-modal-actions">
                    <button type="button" class="btn-modal-cancel"
                        onclick="document.getElementById('editUserModal').classList.remove('active');document.body.style.overflow='auto'">Cancel</button>
